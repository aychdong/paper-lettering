...
def build(results,out):
    ...
    purpose=manifest.get('reviewPurpose','blind')
    data=json.dumps({'evaluationId':evaluation_id,'items':items,'pending':pending,'purpose':purpose},ensure_ascii=False).replace('<','\\u003c')
...
if(data.purpose==='feedback-regression'){document.querySelector('h1').textContent='反馈回看：文案与版式分开选';document.getElementById('status').textContent+=' 这些案例已经评过，后续比较只用于回归，不能计作新的盲测成绩。';}
...
document.getElementById('export').onclick=()=>{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify({version:2,evaluationId:data.evaluationId,votes},null,2)],{type:'application/json'}));a.download='纸上文字-盲评.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),2000)};
    ...
    (out/'盲评.html').write_text(html,encoding='utf-8');(out/'answer-key.json').write_text(json.dumps({'version':2,'purpose':purpose,'evaluationId':evaluation_id,'cases':key,'imageHashes':hashes},indent=2));(out/'status.json').write_text(json.dumps({'ready':len(items),'pending':pending,'human_review':'pending','evaluationId':evaluation_id,'purpose':purpose},indent=2));return len(items)

def verify_images(key,directory):
    for name,expected in key.get('imageHashes',{}).items():
        if Path(name).name!=name or not name.endswith('.png'):raise ValueError('盲评图片路径无效')
        file=directory/name
        if not file.is_file() or hashlib.sha256(file.read_bytes()).hexdigest()!=expected:raise ValueError('盲评图片已变化：'+name)
...
def score(votes,key):
    ...
    records=votes.get('votes',{});cases=key['cases']
    if not isinstance(records,dict) or set(records)-set(cases):raise ValueError('评价包含不属于本轮的案例')
    for v in records.values():
        if not isinstance(v,dict):raise ValueError('评价格式无效')
        for field,allowed in [('copy',['A','B','tie','neither']),('layout',['A','B','tie','neither']),('critical',['none','A','B','both'])]:
            if field in v and v[field] not in allowed:raise ValueError('评价选项无效')
    breakdown={}
    for field in ['copy','layout']:
        counts=dict(creative=0,baseline=0,tie=0,neither=0,pending=0)
        for ident,mapping in cases.items():
            value=records.get(ident,{}).get(field,'pending');counts[mapping.get(value,value)]+=1
        breakdown[field]=counts
    wins={field:breakdown[field]['creative'] for field in ['copy','layout']}
    ...
    passed=len(cases)==12 and complete==12 and wins['copy']>=8 and wins['layout']>=8 and critical==0
    gate='not_applicable_regression' if key.get('purpose')=='feedback-regression' else 'passed' if passed else 'not_met' if len(cases)==12 and complete==12 else 'pending'
    return {'reviewed':complete,'copyWins':wins['copy'],'layoutWins':wins['layout'],'criticalCreativeCases':critical,'breakdown':breakdown,'gate':gate}
if __name__=='__main__':
    ...
    if a.votes:
        key=json.loads(a.key.read_text());verify_images(key,a.key.parent);print(json.dumps(score(json.loads(a.votes.read_text()),key),ensure_ascii=False,indent=2))
