...
function layers(option,doc){
 if(option?.renderLayers){if(!Array.isArray(option.renderLayers)||!option.renderLayers.length||option.renderLayers.length>6)throw new Error('渲染方案无效');return option.renderLayers.map(l=>L.normalizeLayer({...l,id:L.uid()},doc.image.width,doc.image.height));}
